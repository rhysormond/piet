...
/// A Piet program is composed of a grid of colors.
///
...
impl Color {
    ...
    pub fn compare(&self, next: &Color) -> Option<(u8, u8)> {
        match (self, next) {
            (
                ...
                Color::cyclic_distance(hue, next_hue, &HUE_CYCLE_SIZE),
                Color::cyclic_distance(lightness, next_lightness, &LIGHTNESS_CYCLE_SIZE),
            )),
            _ => None,
        }
    }
...
impl From<Rgba<u8>> for Color {
    fn from(from: Rgba<u8>) -> Self {
        match from.to_rgb().channels4() {
            (0xFF, 0xC0, 0xC0, ..) => Color::Color {
                hue: 0,
                lightness: 0,
            },
            (0xFF, 0xFF, 0xC0, ..) => Color::Color {
                hue: 1,
                lightness: 0,
            },
            (0xC0, 0xFF, 0xC0, ..) => Color::Color {
                hue: 2,
                lightness: 0,
            },
            (0xC0, 0xFF, 0xFF, ..) => Color::Color {
                hue: 3,
                lightness: 0,
            },
            (0xC0, 0xC0, 0xFF, ..) => Color::Color {
                hue: 4,
                lightness: 0,
            },
            (0xFF, 0xC0, 0xFF, ..) => Color::Color {
                hue: 5,
                lightness: 0,
            },
            (0xFF, 0x00, 0x00, ..) => Color::Color {
                hue: 0,
                lightness: 1,
            },
            (0xFF, 0xFF, 0x00, ..) => Color::Color {
                hue: 1,
                lightness: 1,
            },
            (0x00, 0xFF, 0x00, ..) => Color::Color {
                hue: 2,
                lightness: 1,
            },
            (0x00, 0xFF, 0xFF, ..) => Color::Color {
                hue: 3,
                lightness: 1,
            },
            (0x00, 0x00, 0xFF, ..) => Color::Color {
                hue: 4,
                lightness: 1,
            },
            (0xFF, 0x00, 0xFF, ..) => Color::Color {
                hue: 5,
                lightness: 1,
            },
            (0xC0, 0x00, 0x00, ..) => Color::Color {
                hue: 0,
                lightness: 2,
            },
            (0xC0, 0xC0, 0x00, ..) => Color::Color {
                hue: 1,
                lightness: 2,
            },
            (0x00, 0xC0, 0x00, ..) => Color::Color {
                hue: 2,
                lightness: 2,
            },
            (0x00, 0xC0, 0xC0, ..) => Color::Color {
                hue: 3,
                lightness: 2,
            },
            (0x00, 0x00, 0xC0, ..) => Color::Color {
                hue: 4,
                lightness: 2,
            },
            (0xC0, 0x00, 0xC0, ..) => Color::Color {
                hue: 5,
                lightness: 2,
            },
...
mod test_codel {
    ...
    fn test_cyclic_distance_identical() {
        assert_eq!(Color::cyclic_distance(&0, &0, &3), 0);
    }

    #[test]
    fn test_cyclic_distance_different() {
        assert_eq!(Color::cyclic_distance(&0, &1, &3), 1);
    }

    #[test]
    fn test_cyclic_distance_cycled() {
        assert_eq!(Color::cyclic_distance(&1, &0, &3), 2);
    }
    ...
    fn test_compare_not_colors() {
        ...
        assert_eq!(color.compare(&Color::Black), None);
        assert_eq!(Color::Black.compare(&color), None);
    }
}
